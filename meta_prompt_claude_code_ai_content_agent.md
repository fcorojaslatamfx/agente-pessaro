...
## 2. Stack técnico (debe replicar el stack existente de Pessaro)

- **Frontend:** React + Vite + TypeScript
- **Backend/DB/Auth:** Supabase (Postgres + RLS + Auth)
- **Deploy:** Vercel
- **Generación de contenido:** API de Anthropic (Claude), llamada desde una función backend (Supabase Edge Function o API route en Vercel — nunca desde el cliente, para no exponer la API key)
- **Cron/scheduling:** Supabase Cron (`pg_cron` + `pg_net`), ya instalados y activos en el proyecto (ver sección 8)
...
- Autenticación contra el **Supabase Auth existente del CRM** (mismos usuarios de `auth.users`; no crear un sistema de auth paralelo ni usuarios duplicados). Roles: `super_admin`, `team_admin`, `advisor`, `team_member`, resueltos leyendo `user_profiles`/`crm_staff_profiles` existentes más la tabla nueva `ai_content_permissions`.
...
| Rol | Puede |
|---|---|
| `advisor` (asesor) | Generar borradores para sus propias RRSS, guardar como borrador, enviar a revisión, ver su propio historial |
| `team_member` (integrante de equipo) | Igual que asesor, más: ver contenido del equipo si tiene permiso |
| `team_admin` (si aplica) | Revisar contenido del equipo, aprobar internamente o escalar a super_admin, ver calendario del equipo |
...
Regla de visibilidad del módulo (referencia, ya que en Fase 2 vivirá dentro del CRM):

```ts
user.role === "super_admin" || permissions.can_access_agent
```
...
3. **Revisión de equipo** (si aplica) → `team_admin` valida tono/coherencia, escala o rechaza con comentarios
...
- `ai_content_permissions` — permisos granulares por usuario, referenciando `user_id → auth.users(id)` y `team_id → crm_teams(id)` (`can_access_agent`, `can_generate_content`, `can_generate_images`, `can_use_public_image_bank`, `can_save_drafts`, `can_submit_for_review`, `can_approve_content`, `can_publish_own_social`, `can_publish_company_social`, `can_view_team_content`, `can_view_company_content`). Esta tabla nueva reemplaza la idea original de agregar columnas a `profiles` — así no se toca ninguna tabla de producción.
...
Todas las tablas deben tener políticas RLS que respeten la jerarquía: `super_admin` ve todo → `team_admin`/miembros ven su equipo si tienen `can_view_team_content` → cada usuario ve/gestiona lo propio por `user_id = auth.uid()`.
...
> **Nota de implementación:** la "instrucción de activación" conversacional (saludar y preguntar el tema) NO aplica aquí — este prompt corre en modo API dentro de la app, no en un chat. El tema y parámetros llegan en el mensaje de usuario construido desde el formulario o el cron. Si se habilita la búsqueda web de la API de Anthropic en la Edge Function, el agente puede investigar fuentes reales; si no, debe basarse solo en conocimiento verificable y marcar cualquier dato que requiera verificación.
...
1. Habilitar las extensiones `pg_cron` y `pg_net` en el proyecto Supabase.
2. Crear una **Edge Function** (`generate-weekly-drafts`) que:
   - Obtenga la lista de asesores/integrantes con `can_use_ai_content = true` y `ai_content_permissions.can_generate_content = true`.
   - Llame a la API de Anthropic con el system prompt de la sección 7, una vez por asesor (o por lote, según volumen).
   - Parsee la respuesta y guarde `ai_content_requests` + `ai_content_outputs` con estado `pending_review`.
   - Esté protegida por un secreto (`Authorization: Bearer <CRON_SECRET>` almacenado en Supabase Vault), para que solo pg_cron (o un llamado autorizado) pueda invocarla.
3. Programar el job en `pg_cron` vía SQL (ejemplo, ajustando el offset horario de `America/Santiago` a UTC):
...
select cron.schedule(
  ...
  select net.http_post(
    url := 'https://<project-ref>.functions.supabase.co/generate-weekly-drafts',
    headers := jsonb_build_object('Authorization', 'Bearer ' || vault_secret('CRON_SECRET'))
  );
  $$
);
```
...
- **Etapa 2:** flujo por equipos (`team_id`, revisión por `team_admin`), biblioteca de contenidos aprobados, calendario editorial por equipo, banco de imágenes públicas/generadas, métricas básicas.
...
3. Las tablas `ai_content_*`, las Edge Functions y el cron **no se tocan** — ya viven donde deben vivir.
...
2. Inicializar el proyecto (Vite + React + TS), configurar el cliente Supabase apuntando a `ldlflxujrjihiybrcree` y variables de entorno.
...
4. Implementar autenticación contra el auth existente y guard de roles (frontend + backend).
5. Construir el formulario de generación de contenido.
6. Implementar la Edge Function que llama a la API de Anthropic con el system prompt de la sección 7 y parsea la respuesta.
...
13. Probar el flujo completo con 2–3 usuarios de prueba (asesor, team_admin, super_admin) antes de desplegar a `agente.pessaro.cl`.
